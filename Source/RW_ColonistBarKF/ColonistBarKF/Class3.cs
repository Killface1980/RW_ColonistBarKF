...
namespace ColonistBarKF.Detouring
{
    ...
    class _SelectorHarmony
    {
        ...
        static class SelectUnderMouse_Pretfix
        {
            ...
            public static void SelectUnderMouse(Selector __instance)
            {
                ...
                else if (list.Count == 1)
                {
                    ...
                    {
                        __instance.ClearSelection();
                        __instance.Select(obj3, true, true);
                    }
                    else if (!__result.Contains(obj3))
                    {
                        __instance.Select(obj3, true, true);
                    }
                    else
                    {
                        __instance.Deselect(obj3);
                    }
                }
                else if (list.Count > 1)
                {
        ...
        static class SelectAllMatchingObjectUnderMouseOnScreen_Pretfix
        {
            [HarmonyPrefix]
            [Detour(typeof(Selector), bindingFlags = BindingFlags.Instance | BindingFlags.NonPublic)]
            private static void SelectAllMatchingObjectUnderMouseOnScreen(Selector __instance)
            {
                List<object> list = SelectableObjectsUnderMouse().ToList<object>();
                if (list.Count == 0)
                {
                ...
                if (clickedThing != null)
                {
                    ...
                    {
                        if (t.def != clickedThing.def || t.Faction != clickedThing.Faction || __instance.IsSelected(t))
                        {
                            return false;
                        }
                ...
                {
                    if (!__instance.IsSelected(current))
                    {
        ...
        static class SelectInsideDragBox_Pretfix
        {
            [HarmonyPrefix]
            [Detour(typeof(Selector), bindingFlags = BindingFlags.Instance | BindingFlags.NonPublic)]
            private static void SelectInsideDragBox(Selector __instance)
            {
                if (!ShiftIsHeld)
                {
                    __instance.ClearSelection();
        ...
        [DebuggerHidden]
        private static IEnumerable<object> SelectableObjectsUnderMouse()
        {
